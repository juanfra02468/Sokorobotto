...
int sim::heuristica(Posicion caja, Posicion objetivo, Tablero* nuevo, bool intercambio) {
    ...
    heuristica = heuristica - 3 * nuevo -> numCajasColocadas();                   
...
bool Algoritmo::escaladaSimple() {
    ...
    Tablero* estado = nullptr, *padre = nullptr;
    bool acabado = false, intercambio, posible, solucion, movido;
    int heuActual, heuCal;

    // Iniciamos la heuristica al valor entre el robot. la primera caja y primer objetivo
    heuActual = sim::heuristica(tablero -> getCajas() -> at(0), tablero -> getObjetivos() -> at(0), tablero, false);
        ...
        if(tablero -> esSolucion()) {
            ...
            for(auto caja = cajas -> begin(); caja != cajas -> end() && !movido; caja++) {
                for(auto objetivo = objetivos -> begin(); objetivo != objetivos -> end() && !movido; objetivo++) {
                    for(int opcion = 0; opcion < 5 && !movido; opcion++) {
                        ...
                        if(posible) {
                            ...
                            heuCal = sim::heuristica(*caja, *objetivo, estado, intercambio);
