...
int bus::heuristica(Posicion caja, Posicion objetivo, Tablero* nuevo, bool intercambio) {
...
bool Algoritmo::busquedaPrimero() {
    ...
    abiertos.push({bus::heuristica(tablero -> getCajas() -> at(0), tablero -> getObjetivos() -> at(0), tablero, false), tablero});
            ...
            for(auto objetivo = objetivos -> begin(); objetivo != objetivos -> end(); objetivo++) {
                for(auto opcion = 0; opcion < 5; opcion++) {
                    ...
                    if(posible) {
                        ...
                        heu = bus::heuristica(*caja, *objetivo, estado, intercambio);                 
                        nuevo = {heu, estado};
